...
#define MAX_PKT_LEN		0x20*4

#define TEST_START_VALUE	0xFB
...
static int CheckData(void)
{
	...
	Value = TEST_START_VALUE + ((TEST_START_VALUE + 1)<<8) + ((TEST_START_VALUE + 2)<<16) + ((TEST_START_VALUE + 3)<<24);
	...
	int tempIndex = 0;
	for(Index = 0; Index < MAX_PKT_LEN; Index ++) {
		//IndexValue = RxPacket[Index] + (RxPacket[Index+1]<<8) + (RxPacket[Index+2]<<16) + (RxPacket[Index+3]<<24);
		if (RxPacket[Index] != (u8)(Value >> (tempIndex * 8)) + 5) {
			xil_printf("Data %u: %x/%x --Error\r\n",
					Index, (unsigned int)RxPacket[Index],
					(u8)(Value >> (tempIndex * 8)) + 5);
			return XST_FAILURE;
		}
		else {
			xil_printf("Data %u: %x/%x\r\n",
					Index, (unsigned int)RxPacket[Index],
					(u8)(Value >> (tempIndex * 8)) + 5);
		}
		Value = Value + (1 << (tempIndex * 8));
