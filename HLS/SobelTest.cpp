...
#define Sobel_Size 3
#define Image_Vertical 50
#define Image_Horizontal 100

// Invert and grayscale image matrix
void Sobel(hls::stream< ap_axiu<32,2,5,6>> &Data_In, hls::stream<ap_axiu<32,2,5,6>> &Data_Out){
	...
	volatile int Array_Image[Image_Vertical*Image_Horizontal*3];
	...
	while(1){
		...
		for(int i = 0; i < 4; i++){
			Array_Image[indx] = (tmp.data.to_int() & Mask) >> (i*8);
			Mask = Mask << 8;
			indx++;
		}
	...
	for(int j = 0; j < Image_Vertical; j++){
		for(int i = 0; i < Image_Horizontal; i++){
			...
			Image[j][i] = R_Weight * R + G_Weight * G + B_Weight * B;
		}
	}
	...
	for(int i = 0; i < Image_Vertical; i++){
		for(int j=0; j < Image_Horizontal; j++){
			if(i == 0 or i == Image_Vertical-1 or j == 0 or j == Image_Horizontal-1){
				...
				G2[0] = Horizontal_Sobel[0][0]*Image[i-1][j-1]  + Horizontal_Sobel[0][2]*Image[i-1][j+1];
				G2[1] = Horizontal_Sobel[1][0]*Image[i][j-1]  + Horizontal_Sobel[1][2]*Image[i][j+1];
				G2[2] = Horizontal_Sobel[2][0]*Image[i+1][j-1] + Horizontal_Sobel[2][2]*Image[i+1][j+1];
				G2_Sum = G2[0]+G2[2];

				Sobel[i][j] = (int)(std::sqrt(G1_Sum*G1_Sum+G2_Sum*G2_Sum)/4);
				//std::cout << Sobel[i][j] << std::endl;
				tmp.data = Sobel[i][j];
			}
			Data_Out.write(tmp);
		}
